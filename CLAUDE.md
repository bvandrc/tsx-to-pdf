...
- **Markdown is a derivative, not a third render.** The PDF and the preview are the same render; the `.md` cannot be, because everything this package exists to control — the sheet, the columns, the alignment, the spacing — lives in classes Markdown has no way to say. It is Turndown over the rendered page, off by default, and documented as the document's text rather than the document. Turndown is an optional peer dependency imported lazily, for the same reason Playwright is: it parses with a full DOM implementation, 9 MB installed, which no one should carry for an output they have not asked for. Its options are cut to `headingStyle` and `codeBlockStyle` — the Prettier pass afterwards rewrites bullet markers and emphasis to its own regardless, so setting those would be describing an outcome we do not control.
