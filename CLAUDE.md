...
A generator: a document written as JSX, styled with Tailwind, rendered to HTML and generated as a page-exact PDF by headless Chromium — also allows generation of Markdown, PNG, and JPG output. The PDF and the browser preview are the same render, not two approximations of each other.
...
- **`src/__tests__/`** covers what runs without a browser: config discovery and its defaults, the generated tsconfig, the compiled stylesheet, the asset copy, and the Markdown derivative. A case that needs Tailwind to resolve builds its fixture tree under `node_modules/` — Tailwind resolves `@import "tailwindcss"` from the root it is handed, and the system temp dir cannot reach one.
...
- `pnpm test` — Vitest over `src/__tests__/`. `pnpm test:watch`, `pnpm test:coverage`; CI runs `pnpm test:unit`, which is the coverage run.
...
- **Markdown is a derivative, not a third render.** The PDF and the preview are the same render; the `.md` cannot be, because everything this package exists to control — the sheet, the columns, the alignment, the spacing — lives in classes Markdown has no way to say. It is node-html-markdown over the rendered page, off by default, and documented as the document's text rather than the document. It has no `Config` key: like `pdf`, whether to write it is a per-build choice rather than a property of the document, so `build()`'s `markdown` option and the CLI's `--md` are the only ways to ask for it. Unlike Playwright, node-html-markdown is a regular dependency, imported directly rather than lazily — it is small enough (no DOM implementation, just `node-html-parser` and an entity decoder underneath, unlike Turndown's `@mixmark-io/domino`, a full DOM at ~9 MB installed, that this package used before) that gating it behind an optional peer dependency wasn't buying consumers anything. Its only set option is `codeBlockStyle: 'fenced'` — the Prettier pass afterwards rewrites bullet markers and emphasis to its own regardless, so setting those would be describing an outcome we do not control, and the library only ever emits ATX headings.
- **Playwright is imported lazily** inside `getBrowser` (`src/get-browser.ts`), so a build that skips `pdf`, `png` and `jpg` works with no browser installed at all.
- **Point-based measurements**: sizes are in points because the output is paper, not a screen. `html { font-size: 12pt }` makes Tailwind's `0.25rem` spacing unit exactly `3pt`, so scale classes land on whole points.
