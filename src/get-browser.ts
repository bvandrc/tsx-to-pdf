...
/**
 * The slice of Playwright's and Puppeteer's own `Page` type this package calls.
 *
 * Both satisfy it structurally, which is what lets `getBrowser` hand back one
 * type regardless of which drove the browser.
 */
export type Page = {
  on: (
    event: 'requestfailed',
...
/**
 * Launches Playwright's browser, or resolves to `undefined` if it isn't installed.
 *
 * `undefined` means the caller should try the next driver. Anything `launch`
 * throws past that point is a real, actionable failure (missing browser
 * binary, bad executablePath, ...), so it is left to propagate rather than
 * being folded into the same "not installed" case.
 */
...
/**
 * Launches the browser: whichever driver `browser` names, or each in turn,
 * Playwright first, when it's unset.
 *
 ...
 * rendered page, ready to generate a PDF or a screenshot.
 *
 * Shared so a build that wants both only launches one browser. The caller
 * owns closing the returned browser.
 */
export const getBrowser = async ({
   ...
   * driver expects.
   *
   * Unset everywhere `playwright install` has run, or under Puppeteer's own
   * downloaded browser.
   */
