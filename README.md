...
- Tailwind scans every `.tsx`/`.ts`/`.jsx`/`.js` file in the entry's directory (recursively), not just the entry itself — so a component split into its own file is still picked up. It also means an unrelated file sitting there gets scanned too; harmless to the PDF or HTML, since an unused class is just dead weight in the stylesheet, but worth knowing if you diff the committed CSS for staleness.
