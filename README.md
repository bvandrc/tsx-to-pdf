...
| `setDate` | `true` | Stamp `/CreationDate` and `/ModDate` with the build time, a given `Date`, or `false` to omit them for a reproducible build. With `true`, a rebuild that renders identical HTML and CSS reuses the previous PDF's date instead of stamping a new one |
